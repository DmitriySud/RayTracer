...
#else
const std::string kArtifactsDir = kBasePath + "dump_res";
#endif
...
TEST_CASE("Vector3D test") {
    {
        Vector3D a(1, 2, 3), b(4, 1, 0), c(7, 6, 1);
        REQUIRE(Vector3D(12, 9, 4) == a + b + c);

        REQUIRE(22 == a * c);
        REQUIRE(a * b == b * a);

        REQUIRE((a * 0.2).x == Approx(0.2));
        REQUIRE((a * 0.2).y == Approx(0.4));
        REQUIRE((a * 0.2).z == Approx(0.6));

        REQUIRE(Vector3D(-3, -5, -1) == b - c);

        REQUIRE(Approx(std::sqrt(14)) == a.GetModule());

        REQUIRE(Approx(std::sqrt(19)) == Vector3D::Distance(a, b));

        REQUIRE(Vector3D(1, -4, 17) == (b ^ c));
        REQUIRE((b ^ c) == -(c ^ b));
    }
}
...
TEST_CASE("Triangle test"){
    {
        ...
        REQUIRE(Approx(2*std::sqrt(3)) == t.GetSquare());

        REQUIRE(t.Intersect({0.5, 0.5, 0.5}));
        REQUIRE(t.Intersect({1, 2, 3}));
        REQUIRE(t.Intersect({1, 1, 1}));
        REQUIRE_FALSE(t.Intersect({-1, 8, 16}));
        REQUIRE_FALSE(t.Intersect({-1, -1, -1}));
    }
}